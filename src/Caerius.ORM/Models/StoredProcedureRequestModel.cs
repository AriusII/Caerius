...
public sealed record StoredProcedureRequestModel(string ProcedureName)
{
    public string ProcedureName { get; } = ProcedureName;
    public List<SqlParameter> Parameters { get; } = [];
    public StoredProcedureRequestModel AddStoredProcedureParameter(string name, object value, SqlDbType type)
    {
        Parameters.Add(new SqlParameter(name, type) { Value = value });
        return this;
    }

    public StoredProcedureRequestModel AddTableValuedParameter<T>(string parameterName, string tvpName, IEnumerable<T> items)
